...
def test_default_flavor(
    ...
    context = load_cmap_context(glyphs_dir, font_size, glyph_scope)
...
def test_top_right_padding(
    ...
    context = load_cmap_context(glyphs_dir, font_size, 'common')
            ...
            if glyph_scope == 'common' or glyph_scope == 'monospaced':
                ...
                match east_asian_width:
                    ...
                    case _:  # Ambiguous (A) or Neutral (N)
                        if glyphs_dir != path_define.POKE_GLYPHS_DIR or code_point not in (
                                0x2640, 0x2642,
                        ):
